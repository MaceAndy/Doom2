...
class ObjectHandler:
    def __init__(self, game):
        ...
        self.npc_types = [SoldierNPC, CacoDemonNPC, CyberDemonNPC, QuakeDemonNPC]
        self.weights = [70, 20, 10, 5]
    ...
    def spawn_npc2(self):
        for i in range(self.enemies):
            npc2 = choices(self.npc_types, self.weights)[0]
            pos = x, y = randrange(self.game.map.cols), randrange(self.game.map.rows)
            while (pos in self.game.map.world_map) or (pos in self.restricted_area):
                pos = x, y = randrange(self.game.map.cols), randrange(self.game.map.rows)
            self.add_npc(npc2(self.game, pos=(x + 0.5, y + 0.5)))
